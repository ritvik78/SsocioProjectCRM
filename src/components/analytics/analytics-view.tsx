...
import { CAMPAIGN_STATUS_COLORS, CAMPAIGN_STATUSES, RESPONSE_TYPE_LABELS } from "@/lib/constants";
...
export function AnalyticsView() {
  ...
  const renderDonut = (rows: { name: string; color: string; count: number }[], suffix: string) => (
    <div className="flex h-52 items-center justify-center">
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          <Pie data={rows.filter((r) => r.count > 0)} dataKey="count" nameKey="name" cx="50%" cy="50%" innerRadius={48} outerRadius={72} paddingAngle={2}>
            {rows.filter((r) => r.count > 0).map((s, i) => (
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="p-5">
        ...
        {emailRows.length === 0 ? (
          <p className="flex h-56 items-center justify-center text-sm text-zinc-400">No emails sent in the last 30 days.</p>
        ) : (
          <div className="mt-3 h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={emailRows} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                <defs>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-5">
          ...
          {campaignRows.every((c) => c.count === 0) ? (
            <p className="flex h-52 items-center justify-center text-sm text-zinc-400">No campaigns yet.</p>
          ) : (
            <div className="mt-3 h-52">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={campaignRows} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e4e4e7" vertical={false} />
